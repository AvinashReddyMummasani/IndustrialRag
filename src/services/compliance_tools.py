...
@tool
def fetch_regulatory_clauses(standard_name: str, equipment_type: str) -> str:
    """
    ...
    try:
        ...
        return json.dumps([{"id": r['clause_id'], "rule": r['requirement'], "limits": r['thresholds']} for r in results])
    except Exception as e:
        logger.error(f"Failed to extract regulatory clauses: {e}")
        return f"Graph DB Error: {str(e)}"

@tool
def fetch_recent_inspections(asset_id: str, limit: int = 3) -> str:
    """
    ...
    try:
        ...
        logger.error(f"Failed to fetch inspection records: {e}")
