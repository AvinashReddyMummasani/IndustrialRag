...
class PostgresPool:
    ...
    async def initialize(cls):
        """Initializes the async connection pool and bootstraps schemas."""
        if not cls._pool:
            dsn = os.getenv("DATABASE_URL")
            if not dsn:
                raise ValueError("DATABASE_URL environment variable is missing.")
            ...
            await cls._bootstrap_db()

    @classmethod
    @asynccontextmanager
    async def get_connection(cls):
        """Async context manager providing transactional connection lending."""
        if not cls._pool:
            raise RuntimeError("Database connection pool uninitialized. Call initialize() first.")

        async with cls._pool.acquire() as conn:
            # Native asyncpg transaction management
            async with conn.transaction():
                yield conn

    @classmethod
    async def _bootstrap_db(cls):
        """Executes strict schema migrations and vector optimization indexing."""
